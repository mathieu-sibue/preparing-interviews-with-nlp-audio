...
class LoadFile(object):
    ...
    def load_document(self, input, model, **kwargs):
        ...
        if is_corenlp(input):
            ...
            parser = RawTextReader(language=language)
            doc = parser.read(text=input, spacy_model=model, **kwargs)
        else:
    ...
    def get_n_best(self, n=10, redundancy_removal=False, stemming=False, words_to_avoid=[]):
        ...
        while len(n_best) != n and idx < len(best):
            u = best[idx]
            unstemmed_word = ' '.join(self.candidates[u].surface_forms[0]).lower()
            if unstemmed_word in words_to_avoid:
                pass
            else:
                n_best.append((unstemmed_word, self.weights[u]))
            idx+=1
